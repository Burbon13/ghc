...
ghcAutogen lbi@LocalBuildInfo{..} = do
  ...
  -- Copy ghcautoconf.h from RTS
  [rtsIncludeDir] <- lines <$> getProgramOutput normal ghcPkg ["--simple-output","--expand-pkgroot","field","rts","include-dirs"]
  copyFile (rtsIncludeDir </> "ghcautoconf.h") (compilerRoot </> "ghcautoconf.h")
