...
expand_syn tvs rhs tys
  ...
  -- This is both more efficient, and steers clear of an infintite
  -- loop; see Note [Care using synonyms to compress types]
  | null tys  = if null tvs then Just rhs else Nothing
  | null tvs  = Just (mkAppTys rhs tys)

  | otherwise = go empty_subst tvs tys
  where
    empty_subst = mkEmptyTCvSubst in_scope
    in_scope = mkInScopeSet $ shallowTyCoVarsOfTypes $ tys
    ...
    go subst [] tys
      | null tys  = Just rhs'  -- Exactly Saturated
      | otherwise = Just (mkAppTys rhs' tys)
    ...
    go _     (_:_)    []       = Nothing  -- Under-saturated
