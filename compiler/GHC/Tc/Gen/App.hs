...
tcInstFun do_ql inst_final (rn_fun, fun_ctxt) fun_sigma rn_args
  ...
  where
    ...
    -- [EDA] TODO: May need to implement this logic for EDictValArg
    ...
    go1 delta acc so_far fun_ty
      ...
      = do { traceTc "go1 EValArg" (vcat [ text "delta:" <+> ppr delta
                                             , text "args:"   <+> ppr rn_args
                                             , text "so_far:" <+> ppr so_far
                                             , text "do_ql:"  <+> ppr do_ql
                                             , text "eva"     <+> ppr eva ])
          ; (wrap, arg_ty, res_ty) <- matchActualFunTySigma herald
                                          (Just (ppr rn_fun))
    ...
    -- This is not correct ... it breaks the type checks (line 1552 in tc dump)
    go1 delta acc so_far fun_ty
            (eva@(EDictValArg { eva_arg = ValArg arg, eva_ctxt = ctxt })  : rest_args)
          = do { traceTc "go1 EDictValArg" (vcat [ text "delta:" <+> ppr delta
                                           , text "args:"   <+> ppr rn_args
                                           , text "so_far:" <+> ppr so_far
                                           , text "do_ql:"  <+> ppr do_ql
                                           , text "eva"     <+> ppr eva ])
