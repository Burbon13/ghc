...
called a RewriterSet. Let's continue our first example above:
...
A RewriterSet is just a set of unfilled coercion holes. This is sufficient
because only equalities (evidenced by coercion holes) are used for rewriting;
other (dictionary) constraints cannot ever rewrite. The rewriter
(in GHC.Tc.Solver.Rewrite) tracks and returns any Wanteds used in rewriting,
to be added to a constraint's RewriterSet by rewriteEvidence and
rewriteEqEvidence, both in GHC.Tc.Solver.Canonical.
...
Worry: It is possible that *all* remaining wanteds are rewritten by others.
Example: TODO "RAE"
In this case, we disable the suppression and report what we have.
